...
function doSearch() {
    ...
    if (q.length < 1) {
        el.innerHTML = '<div class="search-empty"><i class="fas fa-search"></i><br>Type to search members, coaches, fixtures…</div>';
        return;
    }
    el.innerHTML = '<div class="search-loading"><div class="spinner-border spinner-border-sm me-2"></div>Searching…</div>';
        ...
        fetch('ajax_search.php?q=' + encodeURIComponent(q) + '&limit=10')
            ...
            .then(function(data) {
                ...
                for (var i = 0; i < data.results.length; i++) {
                    ...
                    var iconClass = (r.icon || 'fa-circle').replace('fa-', '');
                    html += '<a class="search-result-item" href="' + escHtml(r.url) + '" onclick="closeSearch()" onmouseenter="searchIndex=' + i + '">';
                    html += '<div class="sri-icon ' + iconClass + '"><i class="fas ' + escHtml(r.icon) + '"></i></div>';
                    html += '<div><div class="sri-title">' + escHtml(r.title) + '</div>';
                    html += '<div class="sri-sub">' + escHtml(r.subtitle) + '</div></div>';
                    html += '<span class="sri-type">' + escHtml(r.type) + '</span>';
                    html += '</a>';
                }
                el.innerHTML = html;
                searchIndex = -1;
...
<!-- ══ SEARCH MODAL ══ -->
<div id="searchModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(11,17,32,.5);backdrop-filter:blur(4px);z-index:1050;" onclick="if(event.target===this)closeSearch()">
    <div style="max-width:560px;margin:12vh auto 0;padding:0 1rem;">
        <div class="modal-content">
            <div class="modal-body">
                <div class="search-input-wrap">
                    <i class="fas fa-search search-input-icon"></i>
                    <input type="text" id="searchInput" placeholder="Search members, coaches, fixtures…" autocomplete="off" oninput="doSearch()">
                </div>
                <div class="search-hint">
                    <kbd>↑</kbd> <kbd>↓</kbd> navigate · <kbd>Esc</kbd> close
                </div>
                <div id="searchResults" class="mt-1">
                    <div class="search-empty"><i class="fas fa-search"></i><br>Type to search members, coaches, fixtures…</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0 gap-lg-1">
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <div class="dropdown-header-custom" style="font-size:.68rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;padding:.4rem .85rem .15rem;">People</div>
                        ...
                        <div class="dropdown-header-custom" style="font-size:.68rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;padding:.4rem .85rem .15rem;">Facilities & Gear</div>
                        ...
                        <li><hr class="dropdown-divider"></li>
                        <div class="dropdown-header-custom" style="font-size:.68rem;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;padding:.4rem .85rem .15rem;">Bookings & Maintenance</div>
